...
@section('title', 'List Pengajuan Transfer')
@section('content')
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="card">
                            <div class="card-block">
                                @if($isActive)
                                <a href="{{ route('todo-transfer.create') }}" class="btn btn-primary mb-3">Tambah Pengajuan Transfer</a>
                                @else
                                <a id="tutup-transfer" class="btn btn-secondary mb-3">Tambah Pengajuan Transfer</a>
                                @endif
                                <hr>
                                <div class="row">
                                    <div class="col-md-3">
                                        ...
                                        <select id="filter-status" class="form-select">
                                            ...
                                            <option value="4">Proses Transfer</option>
                                            <option value="5">Sudah Ditransfer</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                ...
                                <div class="dt-responsive table-responsive">
                                    <table id="simpletable" class="table table-striped table-bordered nowrap" style="width: 100%;">
                                        ...
                                        <tr data-status="{{ $todo->status }}" data-date="{{ $todo->tgl }}">
                                            ...
                                            <td>{{ $todo->bank }}</td>
                                            <td>{{ number_format($todo->nominal, 0, ',', '.') }}</td>
                                            <td>
                                            @if($todo->status == 0)
                                                ...
                                                @if($roleUser=='admin')
                                                    @if(!in_array($todo->status, [3, 5])) 
                                                        @if($todo->status == 1)
                                                            ...
                                                            <form action="{{ route('todo-transfer.updateStatus', ['id' => $todo->id, 'status' => 2]) }}" method="POST" class="d-inline confirm-form">
                                                                @csrf
                                                                @method('PUT')
                                                                <button type="submit" class="btn btn-warning">Revisi</button>
                                                            </form>

                                                            <form action="{{ route('todo-transfer.updateStatus', ['id' => $todo->id, 'status' => 3]) }}" method="POST" class="d-inline confirm-form">
                                                                @csrf
                                                                @method('PUT')
                                                                <button type="submit" class="btn btn-danger">Tolak</button>
                                                            </form>

                                                            <form action="{{ route('todo-transfer.updateStatus', ['id' => $todo->id, 'status' => 4]) }}" method="POST" class="d-inline confirm-form">
                                                                @csrf
                                                                @method('PUT')
                                                        ...
                                                        @elseif($todo->status == 4)
                                                            <form action="{{ route('todo-transfer.updateStatus', ['id' => $todo->id, 'status' => 5]) }}" method="POST" class="d-inline confirm-form">
                                                                @csrf
                                                                @method('PUT')
    ...
    <script>
        $(document).ready(function () {
            $("#export-excel").on("click", function (e) {
                ...
                window.location.href = "{{ route('todo-transfer.export') }}?" + query;
            });
        });
    </script>
    ...
    <script>
        $(document).ready(function () {
            $("#tutup-transfer").on("click", function (e) {
                alert("Pengajuan Transfer sedang ditutup.");
            });
        });
    </script>
